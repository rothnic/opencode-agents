...
describe('File Location Check', () => {
  const scriptPath = path.join(__dirname, '../../scripts/gates/file-location-check.ts');
  ...
  describe('Session File Detection', () => {
    ...
    test.each(sessionPatterns)('should detect %s as a session file', filename => {
  ...
  describe('Allowed Root Files', () => {
    const allowedFiles = ['README.md', 'LICENSE', 'package.json', '.gitignore', '.eslintrc.js'];
  ...
  describe('Edge Cases', () => {
    ...
    test('should handle non-git directory gracefully', () => {
      ...
      try {
        ...
        expect(output).toMatch(/All files are in correct locations|Checking file/i);
      } finally {
