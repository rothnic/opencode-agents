...
const { checkStagedFiles } = require("./file-location-check.js");
const { verifyTestEvidence } = require("./test-evidence.js");
...
class GateCheck {
	...
	run(options = {}) {
		...
		const phase = isPhaseCommit
			? commitMsg.match(PHASE_COMMIT_PATTERN)[1]
			: null;
		...
		if (isPhaseCommit && !options.skipTests) {
			...
			console.log(
				"ℹ️  Not a phase completion commit - skipping test requirements\n",
			);
		}
		...
		try {
			...
			this.addResult(
				...
				passed
					...
					: "File location violations found",
			);
		} catch (error) {
			this.addResult(
				GATE_TYPES.FILE_LOCATION,
	...
	checkPhaseRequirements(phase) {
		...
		if (deliverables.length === 0) {
			...
			this.addResult(
				GATE_TYPES.PHASE_REQUIREMENTS,
				true,
				"No specific deliverables to check",
			);
			return;
		}
		...
		if (missing.length > 0) {
			...
			console.log(
				`✅ All deliverables present (${deliverables.length} files)\n`,
			);
			this.addResult(
				GATE_TYPES.PHASE_REQUIREMENTS,
				true,
